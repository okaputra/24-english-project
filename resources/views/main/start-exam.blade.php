    ...
    <div class="container">
        <div class="row g-5">
            <div class="col-lg-8 wow">
                <h1 class="display-6 mb-4">FINAL EXAM</h1>
                ...
                <form method="POST" action="/user-submit-exam/{{$exam['id']}}/{{$id_sub_course}}" id="formJE">
                    @csrf
                    @php
                                    ...
                                    <div class="card">
                                        <div class="card-body">
                                            <p class="card-text"> 
                                                ...
                                                <input type="hidden" class="form-control" id="exampleFormControlInput2" value="{{$currentExam['id']}}" name="id_attempt_exam">
                                                @if($s['audio_file'])
                                                ...
                                                @foreach ($s->opsi as $op)
                                                    <p class="card-text">
                                                        <div class="form-check">
                                                            <input class="form-check-input select_ans_exam" type="radio" name="user_answers[{{$s['id']}}][{{$index+1}}]" id="exampleRadios{{$s['id']}}_{{$index+1}}" value="{{$op['id']}}" @if(in_array($op['id'], $user_answers)) checked @endif>
                                            ...
                                            <div class="input-group">
                                                <textarea rows="7" class="form-control answer_description_exam" name="user_answers[{{$s['id']}}][{{$index+1}}]" aria-label="With textarea">{{$userAnswerDesc ? $userAnswerDesc['user_answer'] : ''}} </textarea>
                                            </div>
                                            @endif
                                        </div>
                                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('.select_ans_exam').change(function() {
            var formData = $('#formJE').serialize();
            $.ajax({
                type: 'POST',
                url: '{{ route("save-answer-exam") }}',
                data: formData,
                headers: {
...
<script>
    $(document).ready(function() {
        var csrfToken = '{{ csrf_token() }}';
        $('.answer_description_exam').keyup(function() {
            var formData = $('#formJE').serialize();
            $.ajax({
                type: 'POST',
                url: '{{ route("save-answer-exam") }}',
...
<script>
    function startTimer(duration) {
      ...
      var startTime = localStorage.getItem('startTimeExam');
      if (!startTime) {
        startTime = Date.now();
        localStorage.setItem('startTimeExam', startTime);
      }
      ...
      var interval = setInterval(function () {
        ...
        if (--timer < 0) {
          ...
          localStorage.removeItem('startTimeExam');
